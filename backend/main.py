...
# CORS: allow localhost:8000, 127.0.0.1:8000, and any deployed domain
CORS(app, origins=[
    "http://localhost:8000",
    "http://127.0.0.1:8000",
    "http://localhost:3000",
...
def upload_too_large(e):
    logger.warning("Upload rejected — file exceeds 100 MB limit.")
    return jsonify({"status": "error", "message": "File too large. Maximum allowed size is 100 MB."}), 413
...
def upload_csv():
    ...
    if 'file' not in request.files:
        logger.warning("CSV upload attempt with no file part.")
        return jsonify({"status": "error", "message": "No file part"}), 400
    
    file = request.files['file']
    if file.filename == '':
        logger.warning("CSV upload attempt with empty filename.")
    ...
    if file and file.filename.endswith('.csv'):
        try:
            ...
            analytics_engine.update_history(df)
            logger.info(f"CSV upload success: {len(data_list)} records added from {file.filename}")
            ...
            logger.error(f"CSV upload failed for '{file.filename}': {e}", exc_info=True)
...
if __name__ == '__main__':
    logger.info("Starting INDRA backend on http://0.0.0.0:8000")
