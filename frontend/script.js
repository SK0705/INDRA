...
let importedCampaignsData = null; // Store imported campaign data
...
async function analyze(event) {
    ...
    if (!form) {
        ...
        showToast('Internal error: form not initialized. Please refresh.', 'error');
        return;
    }
    ...
    console.log("[analyze] Processing data:", data);
    
    // ==================== IMMEDIATE RESULTS DISPLAY ====================
    // Calculate metrics locally for instant feedback
    ...
    currentResult = { ...data, ...instantResult };
    
    // Display results IMMEDIATELY - no waiting for server!
    displayResults(instantResult, data);
    
    // Show success feedback
    console.log("[analyze] Results displayed immediately!");
    
    // Smooth scroll to results section with enhanced behavior
    setTimeout(() => {
        ...
        if (resultsSection) {
            ...
            // Add highlight animation to the results card
            const resultsCard = document.getElementById('resultsCard');
            if (resultsCard) {
                resultsCard.classList.add('highlight-flash');
                setTimeout(() => {
                    resultsCard.classList.remove('highlight-flash');
                }, 2000);
            }
        }
    ...
    // Send to server in background (non-blocking) for database storage
    setLoading(true);
    ...
    try {
        ...
        console.log("[analyze] Server response status:", response.status);
        ...
        console.log("[analyze] Server validation result:", result);
        
        if (result.error) {
            // If server returns an error, show it but keep displayed results
            console.warn("Server returned error but results are shown:", result.error);
        }

    } catch (error) {
        console.error("Server sync error:", error);
        // Results are already displayed, just log the error
        // User doesn't need to know about background sync failure
    } finally {
        setLoading(false);
    }
}
...
function renderComparisonChart(comparisonData) {
    ...
    const campaigns = comparisonData.campaigns;
...
async function compareCampaigns() {
    ...
    try {
        ...
        showToast('Failed to compare campaigns: ' + e.message, 'error');
    } finally {
...
function displayResults(result, inputData) {
    ...
    // Hide empty state and error state
    ...
    roasValue.textContent = roas.toFixed(2) + 'x';
...
async function analyzeAbTest() {
    ...
    if (!testGroup || !testGroup.trim()) {
        showToast('Please enter a Test Group ID to analyze A/B tests.', 'info');
        return;
    }
    ...
    try {
        ...
        showToast('A/B Test Analysis failed: ' + error.message, 'error');
    }
}
...
async function compareChannels() {
    ...
    try {
        ...
        showToast('Channel Comparison failed: ' + error.message, 'error');
    }
}
...
function animateValue(element, value, suffix = '', duration = 1500) {
    if (!element) return;
    const target = parseFloat(value) || 0;
    let start = null;
    
    const initialText = element.textContent;
    // Try to parse the initial value, default to 0 if it's not a number
    const initialValue = parseFloat(initialText.replace(/[^0-9.-]+/g,"")) || 0;

    const step = (timestamp) => {
        if (!start) start = timestamp;
        const progress = Math.min((timestamp - start) / duration, 1);
        const currentValue = initialValue + (target - initialValue) * progress;
        
        element.textContent = (currentValue >= 0 && target > 0 ? '+' : '') + currentValue.toFixed(2) + suffix;
...
async function saveCampaign() {
    if (!currentResult) {
        showToast('No analysis results to save. Please run an analysis first.', 'error');
        return;
    }
    ...
    try {
        ...
        if (result.success) {
            showToast('Campaign saved! Dashboard updating...', 'success');
            updateDashboard();
        } else {
            showToast('Failed to save campaign: ' + (result.error || 'Unknown error'), 'error');
        }
    } catch (error) {
...
async function generateDetailedReport() {
    if (!currentResult) {
        showToast('No analysis results to generate report. Please run an analysis first.', 'error');
        return;
    }
    ...
    try {
        ...
        if (result.success) {
            ...
            showToast('Failed to generate report: ' + (result.error || 'Unknown error'), 'error');
        }
    } catch (error) {
...
async function exportSelectedCampaigns() {
    if (selectedCampaigns.size === 0) {
        showToast('Please select campaigns to export.', 'info');
        return;
    }
    ...
    try {
        ...
        if (response.ok) {
            ...
            showToast('Failed to export selected campaigns.', 'error');
        }
    } catch (error) {
...
async function exportAllCampaigns() {
    try {
        ...
        if (response.ok) {
            ...
            showToast('Failed to export all campaigns.', 'error');
        }
    } catch (error) {
        console.error("Export error:", error);
        showToast('Failed to export: ' + error.message, 'error');
...
async function handleFileUpload(event) {
    ...
    try {
        ...
        if (result.success) {
            // Store imported campaigns data for analysis
            importedCampaignsData = result;
            
            showToast(`${result.imported} campaigns imported! Click "Analyze Now" to process.`, 'success');
            
            // Navigate to main content for analysis
            dashboardSection.style.display = 'none';
            historySection.style.display = 'none';
            comparisonSection.style.display = 'none';
            mainContent.style.display = 'block';
            
            // Focus on the form for user to fill/confirm details
            if (inputs.campaignName) {
                inputs.campaignName.focus();
            }
            
            // Optionally update dashboard with new data
            updateDashboard();
            loadHistory();
        } else {
...
function startTour() {
    ...
    showToast('Welcome to INDRA! This tour will guide you through the features.', 'info');
}
